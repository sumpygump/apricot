...
class Qi_Db_Hazelplum
{
    ...
    private function _dbd_parse_data()
    {
        ...
        if ($dbd_data) {
            ...
            for ($line=0;$line<count($dbd);$line++) {
                ...
                $value = trim(substr($dbd[$line], 0, 4));
                switch ($value) {
                case 'TAB ':
     ...
     * @param string $addl Additional message to accompany error
     * @return void
     */
    private function _throw_error($err, $addl='')
    {
        switch ($err) {
        case 1:
